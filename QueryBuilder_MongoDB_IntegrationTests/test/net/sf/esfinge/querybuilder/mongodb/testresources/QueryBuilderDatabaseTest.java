...
public class QueryBuilderDatabaseTest {
	...
	protected void initializeDatabase(String filename) throws DataSetException, InstantiationException, IllegalAccessException, IllegalArgumentException, SecurityException, NoSuchFieldException, InvocationTargetException {
		...
		TestMongoDBDatastoreProvider dsp = new TestMongoDBDatastoreProvider();
		Datastore ds = dsp.getDatastore();
		
		MongoDBEntityClassProvider ecp = new MongoDBEntityClassProvider();
		
		for(String tableName : dataSet.getTableNames()){
			ITable table = dataSet.getTable(tableName);
			for(int row = 0; row < table.getRowCount(); row++){
				Object entity = ecp.getEntityClass(tableName).newInstance();
				for(Column column : table.getTableMetaData().getColumns()){
					...
					if(!encontrou){
						...
						Class<?> foreignObjectClass = ecp.getEntityClass(foreignCollection);
						...
						Object retrieved = ds.find(foreignObjectClass).field(foreignField.getName()).equal(getCorrectFieldValue(foreignField, value)).get();
	...
	public void cleanDatabase() throws Exception {
		new TestMongoDBDatastoreProvider().resetDB();
	}
